...
def generate_random_content(size_bytes: int) -> bytes:
    ...
    while len(content) < size_bytes:
        ...
        text_chunk = ''.join(random.choices(
...
def create_many_small_files(base_dir: Path, num_files: int, 
                          size_bytes: int = 1024) -> List[Path]:
    """
...
def create_deep_directory_structure(base_dir: Path, depth: int, 
                                  files_per_level: int = 5) -> List[Path]:
    """
