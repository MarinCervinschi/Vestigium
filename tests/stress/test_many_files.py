...
class TestManyFiles:
    ...
    def test_add_many_small_files(self, num_files):
        ...
        print(f"\nMany files add ({num_files} files):")
        print(f"  Execution time: {metrics['execution_time']:.2f}s")
        print(f"  Files/second: {num_files / metrics['execution_time']:.2f}")
    ...
    def test_commit_many_files(self, num_files):
        ...
        print(f"\nMany files commit ({num_files} files):")
        print(f"  Execution time: {metrics['execution_time']:.2f}s")
    ...
    @pytest.mark.parametrize("depth,files_per_level", [
    ...
    def test_deep_directory_structure(self, depth, files_per_level):
        ...
        print(f"\nDeep directory add ({total_files} files, {depth} levels):")
        print(f"  Execution time: {metrics['execution_time']:.2f}s")
        print(f"  Files/second: {total_files / metrics['execution_time']:.2f}")
    ...
    def test_binary_files_performance(self, num_files):
        ...
        print(f"\nBinary files add ({num_files} files):")
        print(f"  Execution time: {metrics['execution_time']:.2f}s")
        print(f"  Files/second: {num_files / metrics['execution_time']:.2f}")
    ...
    def test_incremental_adds(self):
        ...
        for data in timing_data:
            print(f"  Batch {data['batch_size']}: {data['files_per_second']:.2f} files/second "
    ...
    def test_status_with_many_files(self):
        ...
        print(f"\nStatus with many files ({num_files} files):")
        print(f"  Execution time: {metrics['execution_time']:.2f}s")
    ...
    def test_mixed_file_operations(self):
        ...
        small_files = create_many_small_files(
            self.repo_dir / "small", 50, 256
        )
        medium_files = create_many_small_files(
            self.repo_dir / "medium", 20, 4096
        )
        binary_files = create_binary_files(
            self.repo_dir / "binary", 30
        )
        ...
        print(f"\nMixed operations ({total_files} files):")
        print(f"  Add time: {add_metrics['execution_time']:.2f}s")
        print(f"  Commit time: {commit_metrics['execution_time']:.2f}s")
        print(f"  Status time: {status_metrics['execution_time']:.2f}s")
        print(f"  Total time: {add_metrics['execution_time'] + commit_metrics['execution_time']:.2f}s")
        
        # Overall performance should be reasonable
        total_time = add_metrics['execution_time'] + commit_metrics['execution_time']
        assert total_time < 20.0, f"Combined operations took too long: {total_time:.2f}s"